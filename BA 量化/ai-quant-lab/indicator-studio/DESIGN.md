...
- `strategies.py`：策略基类、`MACrossStrategy` 和 `TurtleBreakoutStrategy`。
...
- `rqalpha_runner.py`：完整 RQAlpha Plus 策略回测入口与结果解析。
...
- 当前接入 MA 双均线和海龟交易法策略；后续新增策略时需要补对应的 RQAlpha 约定函数。
...
- 新增海龟交易法策略：入场通道突破、退出通道跌破、ATR 保护止损、参数扫描和 RQAlpha Plus 草稿。
