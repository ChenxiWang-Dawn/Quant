(function () {
  ...
  const STRATEGY_DEFAULTS = {
    ...
    turtle_breakout: {
      ...
      firstMax: 120,
      secondMax: 80,
      note: "突破入场通道买入，跌破退出通道或 ATR 止损卖出",
    },
  };
  ...
  const el = {
    ...
    strategyFastLabel: document.querySelector("#strategyFastLabel"),
    strategySlowLabel: document.querySelector("#strategySlowLabel"),
    strategyFastWindow: document.querySelector("#strategyFastWindow"),
    strategySlowWindow: document.querySelector("#strategySlowWindow"),
    strategyAtrWindow: document.querySelector("#strategyAtrWindow"),
    strategyStopAtr: document.querySelector("#strategyStopAtr"),
    turtleParams: document.querySelectorAll(".turtle-param"),
  ...
  function atrSeries(highs, lows, closes, period) {
    const trueRanges = highs.map((high, index) => {
      ...
      return Math.max(high - lows[index], Math.abs(high - prevClose), Math.abs(lows[index] - prevClose));
    });
  ...
  function simulateTurtleStrategy(entryWindow, exitWindow, atrWindow = 20, stopAtr = 2) {
    const closes = valuesOf("close");
    const highs = valuesOf("high");
    const lows = valuesOf("low");
    const atrValues = atrSeries(highs, lows, closes, atrWindow);
    const fee = (Number(el.feeBps.value) || 0) / 10000;
    let cash = 100000;
    let shares = 0;
    let trades = 0;
    ...
    for (let i = 0; i < state.candles.length; i += 1) {
      ...
      const entryHigh = i >= entryWindow ? Math.max(...highs.slice(i - entryWindow, i)) : null;
      const exitLow = i >= exitWindow ? Math.min(...lows.slice(i - exitWindow, i)) : null;
      const atrValue = atrValues[i];

      if (shares > 0) {
        highestClose = Math.max(highestClose, close);
        if (atrValue) {
          const trailingStop = highestClose - stopAtr * atrValue;
          const initialStop = entryPrice - stopAtr * atrValue;
          stopPrice = Math.max(stopPrice ?? initialStop, trailingStop);
        }
        if ((exitLow != null && close < exitLow) || (stopPrice != null && close <= stopPrice)) {
      ...
      if (shares === 0 && entryHigh != null && atrValue && close > entryHigh) {
        shares = (cash * (1 - fee)) / close;
        cash = 0;
        trades += 1;
        entryPrice = close;
        highestClose = close;
        stopPrice = close - stopAtr * atrValue;
      }
    ...
    const totalReturn = equity.length ? equity[equity.length - 1] / equity[0] - 1 : 0;
    const drawdown = maxDrawdown(equity);
    const score = totalReturn + drawdown * 0.65 - Math.max(0, trades - 12) * 0.002;
    return {
      fast: entryWindow,
      slow: exitWindow,
      entry: entryWindow,
  ...
  function renderOptimizeSummary() {
    ...
    const defaults = STRATEGY_DEFAULTS[selectedStrategy()] || STRATEGY_DEFAULTS.ma_cross;
    el.optimizeSummary.innerHTML = [
      summaryItem("最佳组合", `${best.fast} / ${best.slow}`, `${defaults.firstLabel} / ${defaults.secondLabel}，样本 ${state.candles.length} 根K线`),
      ...
      summaryItem("交易次数", `${metricValue(best, "trades", "tradeCount") || 0}`, defaults.note),
    ].join("");
  ...
  function drawSignalMarkers(panel, visible, toX, toY) {
    ...
    const labeled = new Set(events.slice(-4).map((event) => event.index));
    const labelBoxes = [];
    events.forEach((event) => {
      ...
      const marker = drawSignalBadge(panel, x, baseY, event.type);
  ...
  function drawBacktestTradeMarkers(panel, visible, toX, toY) {
    ...
    const labeledDates = new Set(visibleTrades.slice(-4).map((item) => item.normalized.date + item.normalized.side));
    const labelBoxes = [];
    visibleTrades.forEach(({ normalized }) => {
      ...
      const marker = drawTradeBadge(panel, x, y, normalized.side);
      if (labeledDates.has(normalized.date + normalized.side)) {
        drawMarkerLabel(
          ...
          normalized.side === "buy" ? "above" : "below",
          labelBoxes
        );
      }
    });
  ...
  function drawSignalBadge(panel, x, y, type) {
    const color = type === "buy" ? "#d83a2f" : "#07845f";
    const label = type === "buy" ? "买" : "卖";
    const badgeY = clamp(type === "buy" ? y - 22 : y + 22, panel.y + 14, panel.y + panel.h - 14);
    drawMarkerStem(x, y, badgeY, color);
    drawTextBadge(x, badgeY, 13, color, label, "12px");
    drawPriceAnchor(x, y, color);
    return { labelX: x, labelY: badgeY };
  }

  function drawTradeBadge(panel, x, y, side) {
    const color = side === "buy" ? "#9f1717" : "#047857";
    const badgeY = clamp(side === "buy" ? y - 24 : y + 24, panel.y + 14, panel.y + panel.h - 14);
    drawMarkerStem(x, y, badgeY, color);
    drawTextBadge(x, badgeY, 12, color, "成", "11px");
    drawPriceAnchor(x, y, color);
    return { labelX: x, labelY: badgeY };
  }

  function drawMarkerStem(x, y, badgeY, color) {
    ctx.save();
    ctx.strokeStyle = color;
    ctx.globalAlpha = 0.72;
    ctx.lineWidth = 1.4;
    drawLine(x, y, x, badgeY);
    ctx.restore();
  }

  function drawPriceAnchor(x, y, color) {
    ctx.save();
    ctx.beginPath();
  ...
  function drawTextBadge(x, y, radius, color, text, fontSize) {
    ctx.save();
    ...
    ctx.stroke();
    ctx.fillStyle = "#ffffff";
    ctx.font = `700 ${fontSize} sans-serif`;
    ctx.textAlign = "center";
  ...
  function drawMarkerLabel(panel, x, y, title, detail, color, preferred = "above", occupied = []) {
    const text = `${title} ${detail}`;
    ctx.save();
    ctx.font = "12px sans-serif";
    const displayText = text.length > 18 ? `${text.slice(0, 17)}...` : text;
    const width = Math.min(ctx.measureText(displayText).width + 18, 162);
    const height = 24;
    const box = chooseMarkerLabelBox(panel, x, y, width, height, preferred, occupied);
    occupied.push(box);
    ctx.fillStyle = "rgba(255,255,255,0.96)";
    roundRect(box.x, box.y, width, height, 6);
    ctx.fill();
    ctx.strokeStyle = color;
    ctx.lineWidth = 1.3;
    ctx.stroke();
    ...
    ctx.fillText(displayText, box.x + 9, box.y + height / 2 + 0.5);
    ctx.restore();
  }

  function chooseMarkerLabelBox(panel, x, y, width, height, preferred, occupied) {
    const gap = 9;
    ...
    const candidates =
      preferred === "below"
        ? [
            { x: x + 13, y: below },
            { x: x - width - 13, y: below },
            { x: x + 13, y: above },
            { x: x - width - 13, y: above },
          ]
        : [
            { x: x + 13, y: above },
            { x: x - width - 13, y: above },
    ...
    const normalized = candidates.map((box) => ({
      x: clamp(box.x, panel.x + 4, panel.x + panel.w - width - 4),
      y: clamp(box.y, panel.y + 4, panel.y + panel.h - height - 4),
      w: width,
      h: height,
    }));
    return normalized.find((box) => !occupied.some((item) => rectsOverlap(box, item))) || normalized[0];
  }
  ...
  function applyTemplateStrategyParams(templateName) {
    ...
    el.strategySelect.value = "ma_cross";
    updateStrategyControls(false);
  ...
  function selectedStrategy() {
    return el.strategySelect.value || "ma_cross";
  }

  function updateStrategyControls(resetValues = true) {
    const strategy = selectedStrategy();
    const defaults = STRATEGY_DEFAULTS[strategy] || STRATEGY_DEFAULTS.ma_cross;
    el.strategyFastLabel.textContent = defaults.firstLabel;
    el.strategySlowLabel.textContent = defaults.secondLabel;
    el.strategyFastWindow.min = defaults.firstMin;
    el.strategyFastWindow.max = defaults.firstMax;
    el.strategySlowWindow.min = defaults.secondMin;
    el.strategySlowWindow.max = defaults.secondMax;
    el.turtleParams.forEach((node) => node.classList.toggle("hidden", strategy !== "turtle_breakout"));
    if (resetValues) {
      el.strategyFastWindow.value = defaults.firstDefault;
      el.strategySlowWindow.value = defaults.secondDefault;
      if (strategy === "turtle_breakout") {
        el.strategyAtrWindow.value = 20;
        el.strategyStopAtr.value = 2;
      }
    }
  }
  ...
  function strategyPayload(extra = {}) {
    const strategy = selectedStrategy();
    const targetWeight = clamp(Number(el.targetWeight.value) || 0.95, 0.1, 1);
    const strategyParams =
      strategy === "turtle_breakout"
        ? {
            entry_window: cleanPeriod(el.strategyFastWindow.value, 20),
            exit_window: cleanPeriod(el.strategySlowWindow.value, 10),
            atr_window: cleanPeriod(el.strategyAtrWindow.value, 20),
            stop_atr_multiplier: clamp(Number(el.strategyStopAtr.value) || 2, 0.5, 6),
  ...
  function runLocalBacktest(note) {
    const summary =
      selectedStrategy() === "turtle_breakout"
        ? simulateTurtleStrategy(
            cleanPeriod(el.strategyFastWindow.value, 20),
            cleanPeriod(el.strategySlowWindow.value, 10),
            cleanPeriod(el.strategyAtrWindow.value, 20),
            clamp(Number(el.strategyStopAtr.value) || 2, 0.5, 6)
          )
        : simulateMaStrategy(cleanPeriod(el.strategyFastWindow.value, 5), cleanPeriod(el.strategySlowWindow.value, 20));
    return {
      engine: "local",
      note,
      summary: {
  ...
  async function runStrategyOptimize() {
    try {
      ...
      const results = (payload.results || []).map((item) => ({
        ...item,
        fast: item.fast ?? item.entry,
        slow: item.slow ?? item.exit,
  ...
  function generateStrategyDraft() {
    if (selectedStrategy() === "turtle_breakout") {
      const best = state.optimization.best || {
        fast: cleanPeriod(el.strategyFastWindow.value, 20),
        slow: cleanPeriod(el.strategySlowWindow.value, 10),
      };
      const atrWindow = cleanPeriod(el.strategyAtrWindow.value, 20);
      const stopAtr = clamp(Number(el.strategyStopAtr.value) || 2, 0.5, 6);
      const code = `# AI Quant Lab 策略草稿：海龟交易法
...
def init(context):
    ...
    context.entry_window = ${best.fast}
    context.exit_window = ${best.slow}
    context.atr_window = ${atrWindow}
    context.stop_atr_multiplier = ${stopAtr}
    context.target_weight = ${clamp(Number(el.targetWeight.value) || 0.95, 0.1, 1)}
    context.in_position = False
    context.entry_price = 0
    context.highest_close = 0
...
def handle_bar(context, bar_dict):
    ...
    true_ranges = []
    for idx in range(len(closes) - context.atr_window, len(closes)):
        prev_close = float(closes[idx - 1])
        true_ranges.append(max(float(highs[idx] - lows[idx]), abs(float(highs[idx] - prev_close)), abs(float(lows[idx] - prev_close))))
...
`;
  ...
  function getConfig() {
    return {
      ...
      strategy: {
        ...
        atrWindow: el.strategyAtrWindow.value,
        stopAtr: el.strategyStopAtr.value,
  ...
  function applyConfig(config) {
    ...
    if (config.strategy) {
      ...
      if (config.strategy.atrWindow) el.strategyAtrWindow.value = config.strategy.atrWindow;
      if (config.strategy.stopAtr) el.strategyStopAtr.value = config.strategy.stopAtr;
  ...
  function bindEvents() {
    ...
    el.strategySelect.addEventListener("change", () => {
      updateStrategyControls(true);
      renderOptimizeSummary();
      setStatus(`已切换策略：${el.strategySelect.options[el.strategySelect.selectedIndex].textContent}`);
    });
    [el.strategyFastWindow, el.strategySlowWindow, el.strategyAtrWindow, el.strategyStopAtr, el.targetWeight].forEach((input) => {
