...
from quant_lab import BacktestConfig, optimize_ma_cross, optimize_turtle_breakout, run_backtest as run_quant_backtest, run_rqalpha_strategy
...
def strategy_catalog():
    return [
        ...
        },
        {
            ...
            "name": "海龟交易法",
            "category": "趋势突破",
            "description": "价格突破上一段入场通道买入，跌破短期退出通道或触发 ATR 保护止损时退出。",
            "params": [
                {"key": "entry_window", "label": "入场突破窗口", "type": "number", "min": 5, "max": 120, "step": 1, "default": 20},
                {"key": "exit_window", "label": "退出跌破窗口", "type": "number", "min": 2, "max": 80, "step": 1, "default": 10},
                {"key": "atr_window", "label": "ATR 窗口", "type": "number", "min": 3, "max": 80, "step": 1, "default": 20},
                {"key": "stop_atr_multiplier", "label": "止损 ATR 倍数", "type": "number", "min": 0.5, "max": 6, "step": 0.25, "default": 2.0},
                {"key": "target_weight", "label": "目标仓位", "type": "number", "min": 0.1, "max": 1, "step": 0.05, "default": 0.95},
            ],
        }
    ]
...
def build_backtest_config(payload):
    ...
    if payload.get("strategy") == "turtle_breakout" and "entry_window" not in params:
        params = {
            "entry_window": payload.get("entryWindow", payload.get("fastWindow", 20)),
            "exit_window": payload.get("exitWindow", payload.get("slowWindow", 10)),
            "atr_window": payload.get("atrWindow", 20),
            "stop_atr_multiplier": payload.get("stopAtr", 2.0),
            "target_weight": payload.get("targetWeight", 0.95),
        }
    ...
    if strategy_name not in {"ma_cross", "turtle_breakout"}:
...
def run_backtest(payload):
    ...
    if payload.get("engine") == "rqalpha":
        try:
            return run_rqalpha_strategy(config)
        except Exception as exc:
            if not candles:
...
def explain_research(payload):
    ...
    if strategy_name == "turtle_breakout":
        signal_title = "海龟突破 {}/{} + ATR {}".format(
            params.get("entry_window", 20),
            params.get("exit_window", 10),
            params.get("atr_window", 20),
        )
        signal_body = "海龟策略等待价格突破上一段高点确认趋势，跌破短期低点或 ATR 保护止损退出。重点观察少数大趋势能否覆盖多次小亏损。"
    else:
        signal_title = "MA {}/{} 双均线".format(params.get("fast_window", 5), params.get("slow_window", 20))
        signal_body = "短均线代表较近价格共识，长均线代表中期趋势基准。上穿买入、下穿退出，适合趋势延续行情，震荡市容易反复交易。"
    cards = [
        {
            "tag": "研究结论",
            "title": "趋势策略需要先看稳定性",
...
def best_param_title(optimization):
    ...
    if best.get("entry") and best.get("exit"):
        return "当前较优组合 Entry {} / Exit {}".format(best.get("entry"), best.get("exit"))
...
def strategy_draft_text(payload):
    ...
    strategy_name = payload.get("strategy", payload.get("strategyName", "ma_cross"))
    if strategy_name == "turtle_breakout":
        entry = int(params.get("entry_window", 20))
        exit_ = int(params.get("exit_window", 10))
        atr_window = int(params.get("atr_window", 20))
        stop_atr = float(params.get("stop_atr_multiplier", 2.0))
        target = float(params.get("target_weight", 0.95))
        return f'''# RQAlpha Plus 海龟交易法策略草稿
from rqalpha.api import history_bars, order_target_percent

def init(context):
    context.symbol = "{symbol}"
    context.entry_window = {entry}
    context.exit_window = {exit_}
    context.atr_window = {atr_window}
    context.stop_atr_multiplier = {stop_atr}
...
def run_optimize(payload):
    ...
    if config.strategy_name == "turtle_breakout":
        params = payload.get("strategyParams") or {}
        result = optimize_turtle_breakout(
            candles_to_frame(candles),
            config,
            entry_values=fast_values,
            exit_values=slow_values,
            atr_window=int(params.get("atr_window", payload.get("atrWindow", 20))),
            stop_atr_multiplier=float(params.get("stop_atr_multiplier", payload.get("stopAtr", 2.0))),
