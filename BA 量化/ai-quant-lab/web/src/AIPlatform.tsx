...
function ReinforcementLab() {
  ...
  const run = async () => { try { const item = await api.aiRlRun({ seeds: [7, 17, 29], transactionCost: .001 }); setResult(item); setStatus("多种子仿真已完成；结果只用于研究和影子观察。"); refresh(); } catch (error) { setStatus(error instanceof Error ? error.message : "仿真失败"); } };
  return <Shell title="强化学习实验室" subtitle="先验证环境，再评估受约束的序列决策。当前只提供历史仿真和影子研究，系统没有任何实盘或下单能力。" action={<div className="ai-action-pair"><button className="button button-quiet" onClick={validate}>验证环境</button><button className="button" onClick={run}>运行多种子仿真</button></div>}><div className="ai-toolbar"><span>{status}</span></div>{result && <JsonCard title="最近环境/仿真结果" value={result} />}<AssetTable items={runs} columns={["id", "algorithm", "status", "boundary"]} empty="尚未运行 RL 仿真。" /></Shell>;
}

function DeepLearningLab() {
  const [items, setItems] = useState<Asset[]>([]); const [result, setResult] = useState<Asset | null>(null); const [status, setStatus] = useState("");
  const refresh = () => api.aiDeepLearningRuns().then((data) => setItems(data.runs)).catch(() => setItems([])); useEffect(() => { void refresh(); }, []);
  const run = async () => { try { const item = await api.aiDeepLearningRun({ lookback: 10, epochs: 80, seeds: [7, 17, 29] }); setResult(item); setStatus("本地 MLP 多种子训练已完成；仍需与相同数据和切分下的简单模型比较，才能注册。 "); refresh(); } catch (error) { setStatus(error instanceof Error ? error.message : "训练不可用"); } };
  return <Shell title="深度学习研究基线" subtitle="深度模型先从受控的 MLP 序列基线开始。训练只在本地/受控 Worker 执行，并固定种子、早停、梯度裁剪和张量契约。" action={<button className="button" onClick={run}>运行本地 MLP 基线</button>}><div className="ai-toolbar"><span>{status}</span></div><div className="ai-page-grid"><div className="ai-card"><h2>准入门槛</h2><p>必须先有简单模型基准、严格时间切分和足够样本。每个随机种子独立记录；只选最优种子不构成有效结论。</p><dl className="ai-definition-list"><dt>当前模型</dt><dd>MLP 序列基线</dd><dt>已实现训练纪律</dt><dd>固定种子、验证早停、最佳验证检查点、梯度裁剪。</dd><dt>后续模型</dt><dd>TCN、GRU/LSTM、Transformer 仅在基线 Gate 后进入。</dd></dl></div>{result && <JsonCard title="最近训练结果" value={result} />}</div><AssetTable items={items} columns={["id", "architecture", "status"]} empty="本地尚未训练深度学习基线。" /></Shell>;
}
