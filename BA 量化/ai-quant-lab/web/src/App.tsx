...
function StrategyLab() {
  ...
  const [indicators, setIndicators] = useState<ChartIndicators>({ ma5: true, ma20: true, ema12: false, boll: true, macd: true, rsi: false, kdj: false, atr: false, volma: false });
  if (!strategy) return <NotFound />;
...
type ChartIndicators = { ma5: boolean; ma20: boolean; ema12: boolean; boll: boolean; macd: boolean; rsi: boolean; kdj: boolean; atr: boolean; volma: boolean };
...
function IndicatorPanels({ bars, indicators }: { bars: Candle[]; indicators: ChartIndicators }) {
  const visible = bars.slice(-160); const values = visible.map((bar) => bar.close);
  if (!values.length || (!indicators.macd && !indicators.rsi && !indicators.kdj && !indicators.atr && !indicators.volma)) return null;
  const fast = ema(values, 12).map((value) => value ?? 0); const slow = ema(values, 26).map((value) => value ?? 0); const dif = fast.map((value, index) => value - slow[index]); const dea = ema(dif, 9).map((value) => value ?? 0); const histogram = dif.map((value, index) => (value - dea[index]) * 2);
  const rsi = values.map((_, index) => { if (index < 14) return null; const changes = values.slice(index - 14, index + 1).map((value, position, array) => position === 0 ? 0 : value - array[position - 1]).slice(1); const gain = changes.filter((value) => value > 0).reduce((sum, value) => sum + value, 0) / 14; const loss = Math.abs(changes.filter((value) => value < 0).reduce((sum, value) => sum + value, 0)) / 14; return loss === 0 ? 100 : 100 - 100 / (1 + gain / loss); });
  const rsv = visible.map((bar, index) => { if (index < 8) return null; const sample = visible.slice(index - 8, index + 1); const high = Math.max(...sample.map((item) => item.high)); const low = Math.min(...sample.map((item) => item.low)); return high === low ? 50 : (bar.close - low) / (high - low) * 100; });
  const k: Array<number | null> = []; const d: Array<number | null> = []; rsv.forEach((value, index) => { if (!finite(value)) { k.push(null); d.push(null); return; } const previousK = index ? k[index - 1] : null; const nextK = finite(previousK) ? (2 * previousK + value) / 3 : value; const previousD = index ? d[index - 1] : null; k.push(nextK); d.push(finite(previousD) ? (2 * previousD + nextK) / 3 : nextK); }); const j = k.map((value, index) => finite(value) && finite(d[index]) ? 3 * value - 2 * (d[index] as number) : null);
  const tr = visible.map((bar, index) => Math.max(bar.high - bar.low, Math.abs(bar.high - (visible[index - 1]?.close ?? bar.open)), Math.abs(bar.low - (visible[index - 1]?.close ?? bar.open)))); const atr = sma(tr, 14); const volumes = visible.map((bar) => bar.volume); const volma = sma(volumes, 5);
  return <div className="indicator-panels">{indicators.macd && <MiniIndicator title="MACD (12,26,9)" values={[dif, dea, histogram]} colors={["#315efb", "#dd7a18", "#91a0b3"]} />}{indicators.rsi && <MiniIndicator title="RSI (14)" values={[rsi]} colors={["#7446dc"]} fixedRange={[0, 100]} />}{indicators.kdj && <MiniIndicator title="KDJ (9,3,3)" values={[k, d, j]} colors={["#315efb", "#dd7a18", "#7746d8"]} fixedRange={[0, 100]} />}{indicators.atr && <MiniIndicator title="ATR (14)" values={[atr]} colors={["#168160"]} />}{indicators.volma && <MiniIndicator title="成交量 / VOLMA (5)" values={[volumes, volma]} colors={["#91a0b3", "#315efb"]} />}</div>;
}
...
function IndicatorLibrary() {
  ...
  const formula = formulaByIndicator[selected.id] || formulaByIndicator.ma;
  ...
  return <section className="wrap page indicator-page"><p className="eyebrow">INDICATOR LIBRARY</p><h1>指标百科</h1><p className="page-intro">把技术指标当作观察市场的语言。先理解它在计算什么、适合什么环境，再决定是否把它写入一条可验证的规则。</p><div className="indicator-intro"><div><strong>{indicatorGuides.length} 个已支持指标</strong><span>趋势、动量、波动风险与成交量</span></div><Link className="button button-small" to="/strategies/ma_cross/lab">在行情图中使用指标</Link></div><div className="indicator-tools"><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索 MA、MACD、波动或成交量" aria-label="搜索指标" /><div className="filter-row">{categories.map((item) => <button key={item} className={category === item ? "filter active" : "filter"} onClick={() => setCategory(item)}>{item}</button>)}</div></div><div className="indicator-card-grid">{visible.map((item) => <article className={selected.id === item.id ? "indicator-card selected" : "indicator-card"} key={item.id}><div><span className="tag">{item.category}</span><span className="indicator-code">{item.english}</span></div><h2>{item.name}</h2><p>{item.summary}</p><button className="button button-small button-quiet" onClick={() => openIndicator(item)}>查看指标详情</button></article>)}</div><section id="indicator-detail" className="indicator-detail"><div className="indicator-detail-head"><div><p className="eyebrow">{selected.category.toUpperCase()}</p><h2>{selected.name} <span>{selected.english}</span></h2><p>{selected.summary}</p></div><Link className="text-link" to="/strategies/ma_cross/lab">去行情研究中使用 <span>→</span></Link></div><FormulaPanel formula={formula} /><SelectableStockChart indicator={selected} /><IndicatorTeachingChart indicator={selected} formula={formula} /><div className="indicator-tabs" role="tablist" aria-label="指标内容切换">{tabs.map(([key, label]) => <button key={key} role="tab" aria-selected={tab === key} className={tab === key ? "active" : ""} onClick={() => setTab(key)}>{label}</button>)}</div><div className="indicator-content"><ol>{tabContent.map((item) => <li key={item}>{item}</li>)}</ol>{tab === "decisionGuide" && <aside><strong>使用边界</strong><ul>{selected.cautions.map((item) => <li key={item}>{item}</li>)}</ul></aside>}</div></section><div className="indicator-disclaimer"><strong>重要提醒</strong><p>指标用于研究与复盘，不构成个性化投资建议。任何战法都应在明确标的、样本区间、交易成本和风险约束后进行回测验证。</p></div></section>;
}

function SelectableStockChart({ indicator }: { indicator: IndicatorGuide }) {
  const quickSymbols = [["000001.XSHE", "平安银行"], ["600519.XSHG", "贵州茅台"], ["510300.XSHG", "沪深 300 ETF"], ["AAPL", "Apple"]] as const;
  const [symbol, setSymbol] = useState(() => localStorage.getItem("aiQuantLab.indicatorSymbol") || "000001.XSHE");
  const [bars, setBars] = useState<Candle[]>([]); const [name, setName] = useState(""); const [source, setSource] = useState(""); const [message, setMessage] = useState("输入代码后加载真实 K 线；A 股可用 000001.XSHE，美股可用 AAPL。"); const [busy, setBusy] = useState(false);
  const indicatorState: ChartIndicators = { ma5: indicator.id === "ma", ma20: indicator.id === "ma", ema12: indicator.id === "ema", boll: indicator.id === "boll", macd: indicator.id === "macd", rsi: indicator.id === "rsi", kdj: indicator.id === "kdj", atr: indicator.id === "atr", volma: indicator.id === "volma" };
  const load = async (candidate = symbol) => { const normalized = candidate.trim().toUpperCase(); if (!normalized) { setMessage("请先输入股票或 ETF 代码。"); return; } setBusy(true); setMessage("正在读取真实行情与标的名称…"); try { const data = await api.bars(normalized, yearAgo, today); setBars(data.candles); setName(data.name || data.symbol); setSource(data.source); localStorage.setItem("aiQuantLab.indicatorSymbol", data.symbol); setSymbol(data.symbol); setMessage(`已加载 ${data.candles.length} 根日线。`); } catch (error) { setBars([]); setName(""); setSource(""); setMessage(`暂时无法加载：${error instanceof Error ? error.message : "请检查数据服务设置"}`); } finally { setBusy(false); } };
  return <section className="selectable-stock"><div className="selectable-stock-head"><div><p className="eyebrow">YOUR WATCHLIST</p><h3>用你自选的股票看 {indicator.english}</h3><p>真实行情由已配置的数据服务提供；最近一次查询代码会保留在本机浏览器。</p></div><span className={bars.length ? "real-data-badge" : "real-data-badge idle"}>{bars.length ? "真实行情" : "等待查询"}</span></div><div className="stock-query"><label>股票 / ETF 代码<input value={symbol} onChange={(event) => setSymbol(event.target.value.toUpperCase())} onKeyDown={(event) => { if (event.key === "Enter") load(); }} placeholder="000001.XSHE / 600519.XSHG / AAPL" /></label><button className="button" type="button" disabled={busy} onClick={() => load()}>{busy ? "加载中…" : "加载真实 K 线"}</button></div><div className="quick-symbols">{quickSymbols.map(([code, label]) => <button key={code} type="button" onClick={() => { setSymbol(code); load(code); }}>{label}<span>{code}</span></button>)}</div><p className="stock-query-message">{message}</p>{name && <div className="real-stock-name"><strong>{name}</strong><span>{symbol} · {source} · {bars.length} 根日线</span></div>}{bars.length > 0 && <><MarketChart bars={bars} indicators={indicatorState} trades={[]} /><IndicatorPanels bars={bars} indicators={indicatorState} /></>}<p className="stock-footnote">图表用于研究和学习，不构成任何个性化投资建议。请结合自身风险承受能力，并在策略工作台中进行成本与回测验证。</p></section>;
}

function FormulaPanel({ formula }: { formula: FormulaGuide }) {
  return <section className="formula-panel" aria-label="指标数学公式与变量解释"><div className="formula-panel-copy"><p className="eyebrow">FORMULA, PLAINLY</p><h3>先看懂公式，再看图表</h3><p>公式把指标的计算规则公开出来；下方逐个解释符号，第一次接触也可以顺着读。</p></div><div className="formula-expression">{formula.formulas.map((item) => <div key={item}>{item}</div>)}</div><dl className="formula-variables">{formula.variables.map(([term, description]) => <div key={term}><dt>{term}</dt><dd>{description}</dd></div>)}</dl></section>;
}

function IndicatorTeachingChart({ indicator, formula }: { indicator: IndicatorGuide; formula: FormulaGuide }) {
  const closes = [99, 101, 100, 103, 102, 104, 106, 105, 108, 107, 109, 112, 111, 114, 113, 115, 117, 116, 119, 121];
  const opens = closes.map((close, index) => index === 0 ? 98.5 : closes[index - 1] + (index % 3 === 0 ? 0.5 : -0.35));
  const highs = closes.map((close, index) => Math.max(close, opens[index]) + 1 + (index % 2) * 0.35);
  const lows = closes.map((close, index) => Math.min(close, opens[index]) - .9 - (index % 3) * .2);
  const volumes = closes.map((_, index) => 72 + (index * 11) % 56 + (index > 14 ? 26 : 0));
  const priceMin = Math.min(...lows) - 1;
  const priceMax = Math.max(...highs) + 1;
  const width = 760;
  const priceHeight = 230;
  const slot = width / closes.length;
  const y = (value: number) => priceHeight - 16 - ((value - priceMin) / (priceMax - priceMin)) * (priceHeight - 32);
  const ma5 = sma(closes, 5);
  const ma10 = sma(closes, 10);
  const ema6 = ema(closes, 6);
  const ema12 = ema(closes, 12);
  const bands = bollinger(closes, 7);
  const fast = ema(closes, 6).map((item) => item || 0);
  const slow = ema(closes, 12).map((item) => item || 0);
  const dif = fast.map((item, index) => item - slow[index]);
  const dea = ema(dif, 5).map((item) => item || 0);
  const rsi = closes.map((_, index) => index < 5 ? null : 45 + Math.min(38, Math.max(-30, (closes[index] - closes[index - 5]) * 6)));
  const k = closes.map((_, index) => 30 + ((index * 17) % 55));
  const d = k.map((value, index) => index === 0 ? value : (value + k[index - 1]) / 2);
  const tr = closes.map((_, index) => Math.max(highs[index] - lows[index], Math.abs(highs[index] - (closes[index - 1] || opens[index])), Math.abs(lows[index] - (closes[index - 1] || opens[index]))));
  const atr = sma(tr, 5);
  const volma = sma(volumes, 5);
  const panel = indicator.id === "macd" ? { values: [dif, dea], labels: ["DIF", "DEA"], range: undefined } : indicator.id === "rsi" ? { values: [rsi], labels: ["RSI"], range: [0, 100] as [number, number] } : indicator.id === "kdj" ? { values: [k, d], labels: ["K", "D"], range: [0, 100] as [number, number] } : indicator.id === "atr" ? { values: [atr], labels: ["ATR"], range: undefined } : indicator.id === "volma" ? { values: [volumes, volma], labels: ["成交量", "VOLMA"], range: undefined } : null;
  const allPanel = panel ? panel.values.flat().filter(finite) : [];
  const panelMin = panel?.range?.[0] ?? Math.min(...allPanel, 0);
  const panelMax = panel?.range?.[1] ?? Math.max(...allPanel, 1);
  const panelPath = (values: Array<number | null>) => values.map((value, index) => { if (!finite(value)) return ""; const px = (index / (values.length - 1)) * width; const py = 330 - (((value as number) - panelMin) / Math.max(panelMax - panelMin, .001)) * 62; return `${index === 0 || !finite(values[index - 1]) ? "M" : "L"}${px.toFixed(1)},${py.toFixed(1)}`; }).join(" ");
  return <section className="teaching-chart"><div className="teaching-chart-head"><div><p className="eyebrow">CHART WALKTHROUGH</p><h3>把 {indicator.english} 放回 K 线里看</h3><p>教学示意行情 · 不代表真实标的、实时价格或交易信号。</p></div><span className="demo-badge">研究示例</span></div><div className="teaching-chart-grid"><div className="teaching-plot"><svg viewBox="0 0 760 350" role="img" aria-label={`${indicator.name} 教学 K 线图`}><g className="teaching-grid"><line x1="0" y1="62" x2="760" y2="62" /><line x1="0" y1="142" x2="760" y2="142" /><line x1="0" y1="222" x2="760" y2="222" />{panel && <><line x1="0" y1="278" x2="760" y2="278" /><line x1="0" y1="330" x2="760" y2="330" /></>}</g>{closes.map((close, index) => { const x = index * slot + slot / 2; const up = close >= opens[index]; return <g key={index}><line x1={x} y1={y(highs[index])} x2={x} y2={y(lows[index])} className={up ? "teach-candle teach-up" : "teach-candle teach-down"} /><rect x={x - 7} y={y(Math.max(opens[index], close))} width="14" height={Math.max(2, Math.abs(y(opens[index]) - y(close)))} className={up ? "teach-up" : "teach-down"} /></g>; })}{indicator.id === "ma" && <><path d={linePath(ma5, priceMin, priceMax, width, priceHeight)} className="teach-line teach-blue" /><path d={linePath(ma10, priceMin, priceMax, width, priceHeight)} className="teach-line teach-orange" /></>}{indicator.id === "ema" && <><path d={linePath(ema6, priceMin, priceMax, width, priceHeight)} className="teach-line teach-violet" /><path d={linePath(ema12, priceMin, priceMax, width, priceHeight)} className="teach-line teach-orange" /></>}{indicator.id === "boll" && <><path d={linePath(bands.upper, priceMin, priceMax, width, priceHeight)} className="teach-line teach-dash" /><path d={linePath(bands.mid, priceMin, priceMax, width, priceHeight)} className="teach-line teach-slate" /><path d={linePath(bands.lower, priceMin, priceMax, width, priceHeight)} className="teach-line teach-dash" /></>}{panel && panel.values.map((values, index) => <path key={panel.labels[index]} d={panelPath(values)} className={`teach-line ${index === 0 ? "teach-blue" : "teach-orange"}`} />)}<g className="chart-callout"><rect x="506" y="23" width="210" height="31" rx="7" /><text x="518" y="43">研究提示：先做趋势确认</text></g></svg><div className="teaching-legend">{indicator.id === "ma" && <><span className="legend-blue">MA 5</span><span className="legend-orange">MA 10</span></>}{indicator.id === "ema" && <><span className="legend-violet">EMA 6</span><span className="legend-orange">EMA 12</span></>}{indicator.id === "boll" && <><span className="legend-slate">上 / 中 / 下轨</span></>}{panel && panel.labels.map((label, index) => <span className={index === 0 ? "legend-blue" : "legend-orange"} key={label}>{label}</span>)}</div></div><aside className="chart-lesson"><strong>图中怎么读</strong><ol>{formula.researchSteps.map((step) => <li key={step}>{step}</li>)}</ol><div className="chart-risk"><span>风险边界</span><p>{formula.riskBoundary}</p></div><Link className="button button-small" to="/strategies/ma_cross/lab">拿真实标的验证</Link></aside></div></section>;
}
